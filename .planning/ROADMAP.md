...
Replace Notion API storage with Yandex.Disk WebDAV in three phases: build the WebDAV transport layer, wire up the full content pipeline (markdown, photos, dedup, folder routing), then cut over by removing Notion and deploying. The bot's UX stays identical -- only the storage backend changes.

## Phases

- [ ] **Phase 1: WebDAV Foundation** - HTTP transport layer and vault directory structure on Yandex.Disk
- [ ] **Phase 2: Content Pipeline** - Markdown generation, photo handling, folder routing, and deduplication
...
**Requirements**: WDAV-01, WDAV-02, WDAV-03, WDAV-04, WDAV-05, WDAV-06, FOLD-01, FOLD-03
**Success Criteria** (what must be TRUE):
  1. Bot authenticates with Yandex.Disk and receives a successful response
  2. Bot creates the vault folder structure (Рецепты/, Работа/, Идеи/, Разное/, attachments/) on Yandex.Disk
...
**Depends on**: Phase 1
**Requirements**: MKDN-01, MKDN-02, MKDN-03, MKDN-04, MKDN-05, PHOT-01, PHOT-02, PHOT-03, FOLD-02, DEDU-01, DEDU-02
**Success Criteria** (what must be TRUE):
  ...
  3. Choosing a hashtag category routes the note to the correct folder (e.g. #рецепт goes to Рецепты/)
  4. Sending the same content twice triggers the duplicate detection UI with options to save or cancel
  5. Generated filenames are sanitized and contain date + title (e.g. 2026-03-28_Шарлотка.md)
**Plans**: TBD
